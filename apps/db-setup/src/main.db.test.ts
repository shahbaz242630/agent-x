...
const zitadelLogin = randomBytes(18).toString('hex');
...
function envFor(overrides: Record<string, string> = {}): Record<string, string> {
  return {
    ...
    AGENTX_DB_ZITADEL_PASSWORD: zitadelLogin,
    ...overrides,
  };
}
...
describe(`the set-up job as the compose stack runs it (Postgres ${server.version})`, () => {
  it("creates the app's database and the login service's, keeps the roles, and exits 0", async () => {
    ...
    const logins = [server.admin.password, ...Object.values(server.roles).map((role) => role.password), zitadelLogin];
  ...
  it('left every role able to log in with the login it had', async () => {
    ...
    await expect(
      queryOnce({ ...connection, database: 'zitadel', user: 'zitadel', password: zitadelLogin }, 'select 1 as one'),
