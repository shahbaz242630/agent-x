...
/** The login service's login for this run, random. */
const zitadel = { login: randomBytes(18).toString('hex') };
...
function envFor(overrides: Record<string, string> = {}): Record<string, string> {
  return {
    ...
    AGENTX_DB_ZITADEL_PASSWORD: zitadel.login,
    ...overrides,
  };
}
...
describe(`the set-up job as the compose stack runs it (Postgres ${server.version})`, () => {
  it("creates the app's database and the login service's, keeps the roles, and exits 0", async () => {
    ...
    const logins = [server.admin.password, ...Object.values(server.roles).map((role) => role.password), zitadel.login];
  ...
  it('left every role able to log in with the login it had', async () => {
    ...
    await expect(
      queryOnce({ ...connection, database: 'zitadel', user: 'zitadel', password: zitadel.login }, 'select 1 as one'),
