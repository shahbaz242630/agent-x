...
describe('SEC-OPS-09 each rule can fail', () => {
  ...
  it("no-secret-literals and vault-secrets: each of the app's keys holds its own value and is created once", () => {
    const key = SECRET('key-audit-mac-v1');
    ...
    expect(
      brokenRules(changed(key, (secret) => (inside(secret, 'properties').value = member('key-request-hash-v1')))),
    ).toEqual(['no-secret-literals']);
    expect(brokenRules(changed(key, (secret) => (inside(secret, 'properties').value = 'written in the code')))).toEqual(
      ['no-secret-literals'],
    );
    ...
    expect(
      brokenRules(changed(key, (secret) => (secret.condition = "[not(empty(parameters('appKeyValues')))]"))),
