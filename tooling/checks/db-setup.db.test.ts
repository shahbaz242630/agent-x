...
function start(admin: { user: string; login: string }, roleLogins: SetupLogins) {
...
async function setUp(admin: { user: string; login: string }, roleLogins: SetupLogins) {
  const { run, capture } = start(admin, roleLogins);
...
describe(`the database set-up job on an Azure-like server (Postgres ${server.version})`, () => {
  ...
  it('runs as a superuser too, as on the compose stack, lending itself nothing', async () => {
    const { outcome } = await setUp({ user: 'postgres', login: superuserLogin }, logins);
    expect(outcome).toEqual({ rolesCreated: [], databasesCreated: [] });
