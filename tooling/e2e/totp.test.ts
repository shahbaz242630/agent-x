...
/** RFC 6238 Appendix B: the SHA-1 vectors, whose secret is the ASCII text "12345678901234567890". */
...
describe('TOTP for the authenticator-app test user', () => {
  ...
  it('gives six digits by default, zero-padded', () => {
    expect(totp(RFC_SECRET_BASE32, 1_111_111_109 * 1000)).toBe('081804');
  });

  it('changes with the step and stays the same within it', () => {
    const at = 1_700_000_000_000;
    expect(totp(RFC_SECRET_BASE32, at)).toBe(totp(RFC_SECRET_BASE32, at + 29_999 - (at % 30_000)));
    expect(totp(RFC_SECRET_BASE32, at)).not.toBe(totp(RFC_SECRET_BASE32, at + 60_000));
  });

  it('counts like HOTP (RFC 4226 Appendix D)', () => {
    expect([0n, 1n, 2n, 9n].map((counter) => hotp(RFC_SECRET, counter))).toEqual([
      '755224',
      '287082',
      '359152',
      '520489',
  ...
  it('decodes base32 whatever the case, spacing or padding', () => {
    expect(base32Decode('JBSWY3DPEHPK3PXP').toString('latin1')).toBe('Hello!Þ­¾ï');
