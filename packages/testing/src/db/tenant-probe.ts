...
// transaction's app.org_id setting. The app may read and write it; the backup
...
export async function createTenantProbe(database: TestDatabase): Promise<void> {
  ...
  await owner.query(
    "create policy tenant_isolation on probe.items using (org_id = nullif(current_setting('app.org_id', true), '')::uuid) with check (org_id = nullif(current_setting('app.org_id', true), '')::uuid)",
  );
