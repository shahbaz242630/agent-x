...
//   login afresh, which is how a login is rotated. It changes only databases
//   it made, and never takes Postgres's own or the admin's for the app's.
// - Every connection it opens looks up names in pg_catalog only: the
//   migration role owns the app's database, and could plant a function or
//   operator there that would run with the admin's rights if a query named
//   it by mistake (the CVE-2018-1058 pattern, found by the S13 security
//   review). The check also refuses what could be such a trap.
...
/** Databases the app's can never be: Postgres's own, and the login service's. The admin's is refused too. */
const RESERVED_DATABASES: readonly string[] = ['postgres', 'template0', 'template1', ZITADEL_DATABASE];
...
/** What the app's database reports about its public schema and the admin's place in it. */
export interface PublicSchemaFacts {
  /** Undefined when the database has no public schema. */
  readonly owner: string | undefined;
  /** The rights PUBLIC holds on it. */
  readonly publicPrivileges: readonly string[];
  /** Functions and operators in it: nothing of ours puts any there, and one could stand in for Postgres's own. */
  readonly planted: number;
  /** A schema named after the admin, which `"$user"` would put first on the admin's own search path. */
  readonly adminSchema: string | undefined;
}

/**
 * In the app's database: the owner controls the public schema, if there is
 * one, PUBLIC has no right on it, and nothing waits there, or in a schema
 * named after the admin, for an admin's session to call by mistake (the
 * CVE-2018-1058 pattern: the migration role could plant an operator that runs
 * with the rights of whoever uses it; security review, S13).
 */
export function publicSchemaProblems(database: string, facts: PublicSchemaFacts): string[] {
  const { owner, publicPrivileges, planted, adminSchema } = facts;
  return [
    ...
    ...(planted === 0
      ? []
      : [
          `the public schema of ${database} holds ${String(planted)} functions or operators; nothing of ours puts any there`,
        ]),
    ...(adminSchema === undefined
      ? []
      : [`${database} has a schema named after the admin (${adminSchema}), which would come first on its search path`]),
  ];
}
...
async function connect(options: DatabaseConnectionOptions, logger: Logger): Promise<Client> {
  const client = new pg.Client(adminClientConfig(options));
...
async function securePublicSchema(client: Client, logger: Logger, database: string): Promise<string[]> {
  ...
  // Every comparison is between two values of one type, so Postgres's own operator is the exact match.
  const { rows } = await client.query<{ privilege: string }>(
    `select acl.privilege_type as privilege
     ...
     where n.nspname = 'public' and acl.grantee = 0::pg_catalog.oid
     order by 1`,
  );
  const { rows: counts } = await client.query<{ planted: number; admin_schema: string | null }>(
    `select
       (select pg_catalog.count(*) from pg_catalog.pg_proc p
          join pg_catalog.pg_namespace n on n.oid = p.pronamespace where n.nspname = 'public')::int4
       + (select pg_catalog.count(*) from pg_catalog.pg_operator o
          join pg_catalog.pg_namespace n on n.oid = o.oprnamespace where n.nspname = 'public')::int4 as planted,
       (select nspname::text from pg_catalog.pg_namespace where nspname = current_user) as admin_schema`,
  );
  return publicSchemaProblems(database, {
    owner: await publicSchemaOwner(client),
    publicPrivileges: rows.map((row) => row.privilege),
    planted: counts[0]?.planted ?? 0,
    adminSchema: counts[0]?.admin_schema ?? undefined,
  });
}

/** Who owns a database, or undefined when there is none by that name. */
async function databaseOwner(client: Client, name: string): Promise<string | undefined> {
  const { rows } = await client.query<{ owner: string }>(
...
async function serverProblems(client: Client, database: string): Promise<string[]> {
  ...
  // Our roles as members of anything, and anything as a member of ours but for
  // the one row Postgres 16+ gives the admin that created a role: ADMIN OPTION
  // alone, granted by the bootstrap superuser (always oid 10), to the admin
  // running the job now. A role someone added to zitadel, rights an earlier
  // run lent the admin and never took back, or another admin's rights are
  // what this finds.
  const { rows: memberships } = await client.query<MembershipRow>(
    `select member.rolname as member, granted.rolname as role
     ...
     where member.rolname = any($1)
        or (granted.rolname = any($1)
            and not (member.rolname = current_user and m.grantor = 10::pg_catalog.oid
...
export async function setUpServer(options: ServerSetupOptions): Promise<ServerSetupOutcome> {
  ...
  if ([...RESERVED_DATABASES, options.admin.database].includes(database)) {
    throw new ServerSetupRefused([
  ...
  try {
    ...
    for (const step of bootstrap.databases) {
      const owner = await databaseOwner(admin, step.database);
      const missing = owner === undefined;
      const isApp = step.database === database;
      if (!missing && !isApp) continue;
      // The job changes only a database it made: one by that name that someone else owns is left as it is.
      if (!missing && owner !== step.owner) {
        throw new ServerSetupRefused([`the database ${step.database} is owned by ${owner}, not ${step.owner}`]);
      }
