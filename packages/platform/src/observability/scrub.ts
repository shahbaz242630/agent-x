 ...
 * found. A run can't start inside a longer code, so the digit groups of an ID
 * such as a UUID (`01920000-0000-7000-…`) never count.
 */
const CARD_JOINED = /(?<![0-9A-Za-z-])\d{13,19}(?![0-9A-Za-z-])/g;
const DIGIT_GROUPS = /(?<![0-9A-Za-z-])\d{1,7}(?:[ -]\d{1,7}){2,15}(?![0-9A-Za-z-])/g;
 ...
 * inside a longer code, so an ID's digit groups don't count.
 */
