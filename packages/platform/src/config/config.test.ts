...
describe('SEC-AV-03 config refuses to start when a setting is wrong', () => {
  ...
  it('never repeats a value in a problem, so a secret in the wrong variable does not leak', () => {
    // Plain words, so secret scanners don't mistake the test for a leaked key.
    const secret = 'value-that-must-never-be-printed';
    const env: Env = {
      AGENTX_ENV: secret,
      AGENTX_OUTBOUND_ALLOWED_ORIGINS: `https://api.partner.example,${secret}`,
      AGENTX_PAYEE_COOLING_OFF_HOURS: secret,
      AGENTX_MISSPELT: secret,
      NODE_TLS_REJECT_UNAUTHORIZED: secret,
    };
    const problems = problemsWith(env);
    expect(problems).toHaveLength(5);
    expect(problems.filter((problem) => problem.toLowerCase().includes(secret.toLowerCase()))).toEqual([]);
  });
});
